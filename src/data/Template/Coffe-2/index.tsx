    ...
    <div className="bg-[#f7f9fb] text-[#191c1e] font-inter overflow-x-hidden">
      ...
      <header className="fixed top-0 w-full z-50 shadow-sm bg-[#f7f9fb]/80 backdrop-blur-md">
        <nav className="flex justify-between items-center px-10 py-3 max-w-[1280px] mx-auto">
          ...
          <button className="bg-primary text-white px-6 py-3 rounded-full text-sm font-medium hover:shadow-lg active:scale-95 transition-all">{t.nav.start}</button>
        </nav>
      </header>
      ...
      <main className="overflow-x-hidden">
        ...
        <section className="relative h-screen w-full pt-[72px]" id="hero">
          ...
          <div className="absolute bottom-10 right-10 flex gap-3 z-20">
            <button onClick={() => scrollSlider('prev')} className="w-12 h-12 rounded-full flex items-center justify-center text-[#191c1e] hover:bg-white transition-all" style={glassCard}>
              <span className="material-symbols-outlined">arrow_back</span>
            </button>
            <button onClick={() => scrollSlider('next')} className="w-12 h-12 rounded-full flex items-center justify-center text-[#191c1e] hover:bg-white transition-all" style={glassCard}>
              ...
              <div className="inline-flex p-1 bg-white rounded-full shadow-sm border border-[#c2c6d4]">
                {(Object.keys(t.menuSection.tabs) as TabKey[]).map((tab) => (
                  <button
                    ...
                    className={`px-6 py-3 rounded-full text-sm font-medium transition-all ${activeTab === tab ? 'bg-primary text-white' : 'text-[#424752] hover:bg-[#eceef0]'}`}
                  >
              ...
              {(menuItems ?? []).map((item, i) => (
                <div key={i} className="flex justify-between items-center p-6 rounded-2xl hover:bg-white hover:shadow-md transition-all duration-300 group">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <button className="w-8 h-8 rounded-full border border-[#c2c6d4] flex items-center justify-center hover:bg-secondary hover:text-white transition-all">
