...
- Brand/liên hệ: import từ `src/config/contact.ts` (`BRAND_NAME`, `DOMAIN`, `CONTACT_*`, `FOUNDER_*`). Logo chính thức nằm ở `public/Logo/` (nguồn thiết kế gốc — `.ai`/`.psd`/banner) và `public/logo-wordmark.png` + `public/favicon*`/`public/pwa-*` (asset đã xuất, dùng trong app) — hiển thị qua `src/components/shared/Wordmark.tsx`.
- Màu hệ thống: token trong `src/index.css` (`bg-primary`, `text-on-surface`, `fnb-*`, `viet-*`) — bảng màu "VNGOWEB Pin" (lam ngọc → xanh dương → tím, theo đúng gradient logo/banner chính thức). **Xanh dương/tím giờ LÀ màu hệ thống chính — không quay lại cam/đỏ ấm cũ.** Font cơ bản toàn hệ thống: Inter (`--font-sans`/`--font-display`/`--font-lexend` đều trỏ về Inter, xem comment trong `index.css`). Template trong `src/data/Template/**` giữ màu/font riêng, không đụng.
